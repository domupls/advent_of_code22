...
def multiple_supply_stack():
    ...
    for ins in instructions.split('\n'):
        res = [int(i) for i in ins.split() if i.isdigit()]
        count, from_, to = res[0], res[1], res[2]
        chars = transposed_arr[from_-1][:count]
        transposed_arr[from_-1] = transposed_arr[from_-1][count:]
        transposed_arr[to-1] = chars + transposed_arr[to-1]
    for i in transposed_arr:
        message += i[0]
    return message

print(multiple_supply_stack())